...
## Current state (as of 2026-08-02, batch 15 COMPLETE — extracted, merged, and deployed)

- **Catalogue: 5,911 recipes** (up from 5,295 after batch 14). Production DB verified at 5,922 rows = 5,911 + the established stable +11 offset (harmless legacy rows, see the batch-9 cleanup note above).
- **Fetch limit reached: 2500** (per-channel, newest-first; bumped from 2000, a moderate +500 jump — deliberately smaller than batch 14's +1000 given how large that batch's extraction workload turned out to be, and because several channels were already close to exhaustion). This produced only **1,089 new videos** (vs. 3,609 in batch 14) — four of six channels came back near-exhausted at this depth: YourFoodLab (6 new), RanveerBrar (3 new), KunalKapur (3 new), and KabitasKitchen (477 new, found fewer videos than the limit). Only nishamadhulika (492 new) and sanjeevkapoorkhazana (108 new) hit the full 2500 cap, meaning they still have more content beyond this fetch. Stage B workload: **137 chunks** (chunk_925–1061), a big drop from batch 14's 452.
- **Stage B extraction: all 137 chunks done, 100%, spanning 17 waves.** 1,088 videos processed (1,089 fetched minus 1 raw file missing from disk, see gotcha below), **1,035 `is_recipe:true`** (~95% yield — very high because the small fetch increment kept pulling each channel's easy/recent content rather than deep back-catalog; several waves came back a perfect 8/8 or even 64/64 for the whole wave). Channel boundaries crossed: KabitasKitchen→KunalKapur/RanveerBrar/YourFoodLab (all three's last few videos landed together in chunk_984-985, since each had so few new videos left), YourFoodLab→nishamadhulika (chunk_986), nishamadhulika→sanjeevkapoorkhazana (chunk_1047) — all 6 channels touched again.
- **Stage C merge**: of the full accumulated corpus (8,416 videos across all batches ever extracted), 4,858 were already-merged from prior batches, 1,851 were already-covered (same dish name already in the catalogue under a different channel), 634 were newly accepted, 1,073 dropped as not-a-recipe, 0 rejected. Cross-channel dedup within this run dropped 18 more (kept the most-viewed video per dish name) → **final +616 recipes**. Nutrition: 34 verified against the INDB workbook, 600 fell back to AI-estimated.
- **Deploy**: local `php scripts/seed.php` (5,911 recipes), committed+pushed `recipes.json` (commit `1539fa8`), production `scp` + `seed.php` (5,911 recipes) + `backfill-images.php` (437 recipes populated — these are the same ~437 curated non-video recipes every batch backfills, since `recipes.json` never carries `image_url` for them and every `seed.php` upsert clears it again; this is expected, not new work from this batch). Verified prod row count 5,922 matches 5,911 + 11 offset.
- **New gotcha this batch: a raw video file listed in the Stage B manifest was missing from disk** (`nishamadhulika/o9Nr0z2aiL8.json`, assigned to chunk_1038). The subagent correctly read the other 7 files in its chunk and reported "7 of 7 extracted videos are recipes... file does not exist" rather than erroring out — the final chunk-verification pass confirmed this as the only count mismatch (7 entries vs. 8 manifest files) across all 137 chunks, with no other issues. No action was needed; this doesn't appear to be a systemic problem, just one video that vanished between Stage A fetch and Stage B extraction (possibly deleted/unlisted by the uploader in between).
- **This batch ran far more smoothly than batch 14** — the smaller fetch (1,089 vs 3,609 new videos, 137 vs 452 chunks) meant the whole extraction completed in 17 waves with comparatively few session-limit interruptions, and several waves landed perfect scores (waves 12, 13, and 14 each returned 64/64 `is_recipe:true` across all 8 chunks).
...
### Per-channel totals vs. scraped so far (recounted directly from `raw/` folders, 2026-08-02, after batch 15)

| Channel | Long-form scraped | Total channel uploads (incl. Shorts, via API, as of batch 10 — stale) | Status |
|---|---|---|---|
| YourFoodLab | 1,507 | 1,848 | **Near-exhausted** — only 6 new videos found at `--limit 2500` |
| KabitasKitchen | 1,851 | 2,476 | Still yielding (477 new this batch) |
| nishamadhulika | 2,375 | 2,570 | Hit the fetch cap (492 new) — more remains |
| RanveerBrar | 1,036 | 1,857 | **Near-exhausted** — only 3 new videos found |
| sanjeevkapoorkhazana | 728 | **17,142** | Hit the fetch cap (108 new) — vast remaining depth |
| KunalKapur | 920 | 1,259 | **Near-exhausted** — only 3 new videos found |

**Note:** the numbers above are a fresh direct count of `raw/<channel>/*.json` files, summing to 8,417 (one more than `merge.py`'s `videos=8416` line, because one raw file — `nishamadhulika/o9Nr0z2aiL8.json` — is present in some earlier snapshot logic but was missing from disk during this batch's Stage B extraction; see the gotcha above). Re-derive this way (not from memory/prior doc text) whenever it's next updated. **Three of six channels (YourFoodLab, RanveerBrar, KunalKapur) are now effectively exhausted of long-form content at this fetch depth** — the next batch's fetch will likely return ~0 new videos for these three regardless of `--limit`, until/unless the channels upload new content. Only nishamadhulika and sanjeevkapoorkhazana still have meaningfully more to scrape.

Caveats: total-uploads column is stale (last checked batch 10) and includes Shorts (which we filter, min 90s duration). **sanjeevkapoorkhazana remains the major outlier** on total uploads (17k+ vs. 1–2.4k for the rest) — its batch-15 dose (108 new videos, 39 sanjeevkapoorkhazana-only chunks in the final wave) again shows a lower accepted-recipe yield than the other channels (several chunks in the 6-7/8 range vs. many 8/8 chunks earlier in the batch from KabitasKitchen/nishamadhulika), consistent with every prior batch's pattern. Still the weakest-yield channel by a wide margin, and now the most valuable one to keep dedicating fetch depth to, since the other channels near this current fetch depth are running dry.
