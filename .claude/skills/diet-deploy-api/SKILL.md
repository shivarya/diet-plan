...
4. **Database**:
   ...
   - **Existing/live DB (updates)**: do NOT re-import `schema.sql` (it's `CREATE TABLE IF NOT EXISTS` — won't alter existing tables). Apply the numbered `database/migrations/NNN_*.sql` in order (back up first, e.g. `mysqldump ... | gzip > backup.sql.gz`), then re-run `php scripts/seed.php` if `recipes.json` changed (then `php scripts/backfill-images.php` to populate photos for any newly added recipes — idempotent, only fills empty `image_url`; most YouTube-imported recipes already carry a thumbnail `image_url` so this mainly covers older/INDB-style entries). Migrations so far: `001_food_type_slots_sides.sql`, `002_recipe_details.sql`, `003_dal_per_week.sql` (`dietary_preferences.dal_per_week`), `004_recipe_provenance.sql` (`recipes.nutrition_source`/`source_channel`), `005_dessert_category_and_nutrition_gate.sql` (`recipes.dish_category` adds `dessert`, `dietary_preferences.nutrition_gate_enabled`), `006_shuffle_history.sql` (`meal_plan_items.shuffle_history` — lets `shuffleItem()` exclude recently-shown recipes per slot, not just recipes currently elsewhere in the week's plan). When SSH-ing into a live remote DB, strip `\r` from `.env` values read via `grep`/`cut` (`| tr -d '\r\n'`) — a CRLF-edited `.env` can otherwise silently break `mysql -h "$DB_HOST"`.
